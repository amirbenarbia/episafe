...
void setup() {
  ...
  // Get the MAC address for Bluetooth
  ...
  while (1) {
    if (xQueueReceive(dataQueue, &data, portMAX_DELAY)) {
        ...
        if (!SerialBT.hasClient()) {
          ...
          String receivedTimeStr = SerialBT.readString();
          /*Serial.print("Received Smartphone Time: ");
          Serial.println(receivedTimeStr);*/
          time_t smartphoneTime = receivedTimeStr.toInt();    

          for (int i = 0; i < dataCount; i++) {

            dataString = Query(dataBuffer[i]); 
            /*SerialBT.print("Smartphone Time: ");
            SerialBT.println(smartphoneTime);
            SerialBT.println(receivedTimeStr);*/
